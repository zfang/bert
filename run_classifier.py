...
def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer):
    ...
    return [_get_features(tokenizer, label_map, max_seq_length, ex_index, example)
            for (ex_index, example) in tqdm(enumerate(examples), total=len(examples))]


def _get_features(tokenizer, label_map, max_seq_length, ex_index, example):
    ...
    if ex_index < 5:
        ...
        tf.logging.debug("input_mask: %s" % " ".join([str(x) for x in input_mask]))
        tf.logging.debug(
